...
def login_input():
    """Prompts the user for a username and password, and returns the response in a tuple"""
...
def new_password_input():
    ...
    if password1 == password2:
        ...
        print("You did enter the same password.  Please try again.")

def regex_input(prompt, pattern):
    """Repeatedly prompts the user for an input matching the given regex pattern.
        Return's the user's response"""
    while True:
...
def dialog(heading, data):
    ...
    for item in data:
        ...
        response = regex_input(prompt, pattern)
